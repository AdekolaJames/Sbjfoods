...
export function StockHistoryDialog({ stockItemId, itemName, open, onOpenChange }: Props) {
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Package className="h-5 w-5" /> Stock History — {itemName || ''}
          </DialogTitle>
          <DialogDescription>
            Full audit trail of purchases (with cost) and movements (deductions, adjustments, waste).
          </DialogDescription>
        </DialogHeader>
        ...
        <Tabs defaultValue="purchases">
          <TabsList>
            ...
            <TabsTrigger value="movements">Movements ({movements?.length || 0})</TabsTrigger>
          </TabsList>
                  ...
                  <TableBody>
                    {purchases?.length ? purchases.map(p => (
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="font-medium">₦{Number(p.total_cost).toLocaleString()}</TableCell>
                  ...
                  <TableBody>
                    {movements?.length ? movements.map(m => (
                      <TableRow key={m.id}>
                        ...
                        <TableCell>
                          <Badge variant={actionBadge(m.action) as any} className="capitalize">{m.action}</Badge>
                        </TableCell>
                        <TableCell>{m.quantity}</TableCell>
                        <TableCell className="text-xs">{m.reason || '-'}</TableCell>
                      </TableRow>
                    )) : (
                      <TableRow><TableCell colSpan={4} className="text-center py-8 text-muted-foreground">No movements</TableCell></TableRow>
                    )}
                  </TableBody>
                </Table>
              )}
